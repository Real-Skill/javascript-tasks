...
describe('Lodash training', function ()
{
    describe('Lang functions', function ()
    {
        ...
        describe('isBuffer', function() {
            var params, name, int32A;

            before(function() {
                params = datasets.isBuffer();
                int32A = params[0];
                name = int32A.constructor.name;
            });

            it('should verify params', function() {
                expect(params).to.have.length(1);
                expect(name).to.eql('Int32Array');
                expect(int32A).to.have.length(8);
                expect(int32A instanceof Int32Array).to.eql(true);
            });

            it('should returns false if value is not a buffer ', function() {
                expect(_.isBuffer.apply(_, datasets.isBuffer(params))).to.eql(false);
            });
        });
